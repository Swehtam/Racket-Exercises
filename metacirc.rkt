...
;; avalia uma expressão sequencial?
(define (eval-begin args)
  (define prim (eval (first args)))
  (define others (rest args))
  (cond [(empty? others) prim]
        [else (eval-begin others)]))
...
(adiciona-primitiva '<= <=)
(adiciona-primitiva '>= >=)
(adiciona-primitiva 'empty? empty?)
...
(module+ test
  ...
  (define-test-suite test-eval
    ...
    (test-equal? "sequencial com definição e primitiva *"
                 (eval '(begin (define x 2) (* x 11))) 22)
    (test-equal? "sequencial com definição e primitiva *"
                 (eval '(begin (define x 2) x)) 2)
    (test-equal? "primitiva com sequencial"
                 (eval '(+ 1 (begin (define x 2) (* x 11) 2567))) 2568)
    (test-equal? "testando se o define está salvando a variavel x"
                 (eval-temp-env '((y 2)) '(* x y)) 4))
