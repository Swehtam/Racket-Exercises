...
;; Crie uma função (par n) que retorna #t se n é par e #f se n é par. Em seguida,
;; crie uma função (impar n) que retorna #t se n é ímpar e #f se n é ímpar. Pense em
...
(define (elem3-lista6 list)
  (define (elem3-lista6-acc list acc)
    (if (= acc 1)
        (first list)
        (elem3-lista6-acc (rest list) (- acc 1))))
  (elem3-lista6-acc list 3))


(define-test-suite teste-ex6
  (test-equal? "elem3-lista6 deve ser 33" (elem3-lista6 lista6) 33))
...
(define (soma-lista l)
  (define (soma-lista-acc l acc)
    (if (empty? l)
        acc
        (soma-lista-acc (rest l) (+ acc (first l)))))
  (soma-lista-acc l 0))
...
(define (mult-lista l)
  (define (mult-lista-acc l acc)
    (if (empty? l)
        acc
        (mult-lista-acc (rest l) (* acc (first l)))))
  (mult-lista-acc l 1))
...
(define (max-lista l)
  (define (max-lista-acc l acc)
    (if (empty? l)
        acc
        (if (> (first l) acc)
            (max-lista-acc (rest l) (first l))
            (max-lista-acc (rest l) acc))))
  (max-lista-acc l 0))
...
(define (quadrado-lista l)
  (define (quadrado-lista-acc l acc-l)
    (cond [(empty? l) acc-l] 
          [else (quadrado-lista-acc (rest l) (append  acc-l (list (* (first l) (first l)))))]))
  (cond [(empty? l) '()]
        [else (quadrado-lista-acc l '())]))
...
(define (filtra-par l)
  (define (filtra-par-acc l acc-l)
    (cond [(empty? l) acc-l]
          [(par (first l)) (filtra-par-acc (rest l) (append acc-l (list (first l))))]
          [else (filtra-par-acc (rest l) acc-l)]))
  (cond [(empty? l) '()]
        [else (filtra-par-acc l '())]))
